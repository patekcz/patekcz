...
function formatLanguagesData(languagesData) {
  const languages = languagesData.data.map(lang => {
    ...
    return `<span style="display: inline-flex; align-items: center; font-size: 12px; white-space: nowrap;"> <img src="${iconUrl}" height="20" alt="${lang.name.toLowerCase()} logo" style="margin-right: 4px;" /> **${lang.name.toLowerCase()}**: ${lang.percent.toFixed(2)}%</span>`; // Zajištění jednoho řádku
